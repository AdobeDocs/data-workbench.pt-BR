---
description: Instruções detalhadas para a instalação e configuração do Sensor para WebLogic Server 6.x ou posterior em execução no Microsoft Windows Server 2000 ou posterior.
title: WebLogic Server no Windows Server 2000 ou posterior
...
# WebLogic Server no Windows Server 2000 ou posterior{#weblogic-server-on-windows-server-or-later}

Instruções detalhadas para a instalação e configuração do Sensor para WebLogic Server 6.x ou posterior em execução no Microsoft Windows Server 2000 ou posterior.
...
1. No WebLogic Server, crie um diretório no qual os arquivos de programa do Sensor serão instalados. Lembre-se de que sua fila de discos está nesse diretório, portanto, verifique se o dispositivo escolhido tem espaço suficiente para manter uma fila do tamanho necessário.
...
Instruções para modificação do script de inicialização.

No script usado para iniciar o WebLogic (por exemplo, C:\bea\user_projects\mydomain\startServer.cmd), edite a linha &quot;set JAVA_OPTIONS=&quot; para definir a definição java.library.path para o diretório que contém o arquivo visual_sciences.dll.
