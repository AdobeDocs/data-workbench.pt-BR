---
description: Instruções detalhadas para a instalação e configuração do Sensor para o Apache Jakarta Tomcat 4.1 ou posterior em execução no Windows Server 2000 ou posterior.
title: Servidor Tomcat no Windows Server 2000 ou posterior
...
# Servidor Tomcat no Windows Server 2000 ou posterior{#tomcat-server-on-windows-server-or-later}
...
1. No servidor Tomcat, crie um diretório no qual os arquivos de programa Sensor serão instalados. Lembre-se de que sua fila de discos está nesse diretório, portanto, verifique se o dispositivo escolhido tem espaço suficiente para manter uma fila do tamanho necessário.
...
1. Em bin folder, execute Tomcat9w.exe (Common daemon service manager).
   ...
   Onde o [!DNL C:\Sensor] diretório é o diretório que contém o [!DNL visual_sciences.dll] arquivo.
