---
description: O Adobe Analytics atualizará e aprimorará seu feed de dados acionando o Análise de big data para aproveitar os Perfis e públicos-alvo na Adobe Experience Cloud. Todos os usuários da Análise de big data devem estar preparados para fazer a transição para esse novo formato de feed de dados até 21 de abril de 2014.
title: Atualizações para abril de MR 2014
...
# Atualizações para abril de MR 2014{#updates-for-april-mr}

O Adobe Analytics atualizará e aprimorará seu feed de dados acionando o Análise de big data para aproveitar os Perfis e públicos-alvo na Adobe Experience Cloud. Todos os usuários da Análise de big data devem estar preparados para fazer a transição para esse novo formato de feed de dados até 21 de abril de 2014.

Perfis e públicos-alvo fornecem uma exibição completa e totalmente acionável dos clientes para uso na Adobe Experience Cloud e em serviços de terceiros. Para começar a estabelecer a base para novos recursos no Analytics, o novo Serviço da Experience Cloud ID será adicionado ao feed de dados nesta versão de abril do MR, juntamente com outros aprimoramentos e aprimoramentos. Esse novo serviço está disponível para qualquer cliente que usa a Adobe Experience Cloud.

Aprimoramentos e correções de erros adicionais fornecidos com este novo feed de dados estão listados abaixo neste tópico, bem como instruções para preparar a configuração do Análise de big data.

## Aprimoramentos e correções para o novo feed de dados {#section-0013de07da93411d929a1dfd0417f61b}

* Adição do serviço da Experience Cloud ID.
* Adição do campo Ação do Target para suportar relatórios e análises do Target no Analytics.
* Alinhamento aprimorado com o Report &amp; Analytics ao incorporar o [!DNL bot_id] valor nos tipos de linha do produto.
* Redução no tamanho das eVars de comercialização sem perda de integridade nos valores de dados.
* Solução de um problema em que algumas vezes valores com hash apareciam em eVars de comercialização.
* Melhorias internas no processo para continuar a oferecer melhor estabilidade e desempenho.

## Atualização da configuração da análise de big data {#section-381fcd185af6474bb656501c096c451d}

As alterações na análise de big data garantirão a integração ininterrupta do feed de dados após a transição. Durante a próxima atualização programada de reprocessamento, o Grupo decodificador atual na fase de processamento de log da configuração. A equipe responsável pelo gerenciamento da arquitetura do Análise de big data poderá realizar tarefas adicionais para ajudar a expor e aproveitar o novo identificador de visitante da Experience Cloud.

**Tarefas para atualizar o Análise de big data**

1. Localize o Grupo decodificador atual. Normalmente, localizado no Conjunto de dados\Processamento de log\Decodificação encontrado em um Gerenciador de perfis. [!DNL Instructions.cfg]
1. No arquivo de configuração, clique com o botão direito do mouse no formato do decodificador atual e selecione Copiar.
1. Clique com o botão direito do mouse e cole uma nova entrada abaixo do formato de decodificador atual.
1. Expanda o novo formato de decodificador e adicione duas novas posições de campo ao final da lista. Essas posições podem ser deixadas em branco se você não estiver pronto para usar os novos campos no conjunto de dados, ou defini-los da seguinte forma.

   * [!DNL x-mcvisid]
   * [!DNL x-tnt-action]

## Instruções de decodificação {#section-ba63132ce9604851ad319b6181011820}
